...
import com.nerys.faces.services.DepartamentFacesServices;
...
@ViewScoped
@Data
public class DepartamentFaces implements Serializable, JmoordbCoreXHTMLUtil, IPaginator, DepartamentFacesServices , FacesServices{
    ...
    private Departament departamentSelectedAutocomplete = new Departament();

    private Central centralSelected = new Central();
    private Departament departamentSelected = new Departament();
    private Departament departamentDB = new Departament();
    ...
    private Boolean haveDepartamentOpen = Boolean.FALSE;
    ...
    // </editor-fold>
     ...
     public Departament getDepartamentSelected () {
          if(departamentSelected== null){         
              departamentSelected = new Departament();
        }
    ...
    private LazyDataModel<Departament> departamentLazyDataModel;
// </editor-fold>
...
// </editor-fold>
    ...
    public void init() {
        try {
            haveDepartamentOpen = Boolean.FALSE;
            message = "";
...
departamentSelectedAutocomplete = new Departament();
            findAll();
centralSelected = centralServices.findByIdcentral(userLogged.getCentralView().getIdcentral()).get();
            this.departamentLazyDataModel = new LazyDataModel<Departament>() {
                @Override
                public List<Departament> load(int offset, int pageSize, Map<String, SortMeta> sortBy, Map<String, FilterMeta> filterBy) {

                    switch (paginator.getName()) {
                        case "findAll":

                            totalRecords = departamentServices.count(paginator.getFilter(),
...
//
                    ...
                    switch ((paginator.getName())) {
                        case "findAll":
                            ...
                            result = departamentServices.lookup(paginator.getFilter(),
                ...
                public int count(Map<String, FilterMeta> map) {
                 ...
                 @Override
                public String getRowKey(Departament object) {
                    if (object == null || object.getIddepartament()== null) {
                        return "";
                    }
                    return object.getIddepartament().toString();
                }

                @Override
                public Departament getRowData(String rowKey) {
                    for (Departament t : departamentList) {
                        if (t != null) {
                            if (t.getIddepartament().equals(rowKey)) {
                                return t;
                            }
                        }
                    }
 ...
 */
    ...
    public void preDestroy() {
       ...
       departamentConverterServices.destroyed();
  ...
  Bson filter = and(eq("active", Boolean.TRUE),
            ...
            paginator
                    = new Paginator.Builder()
                            ...
                            .filter(filter)
                            .sort(sort)
                            .sorted(new Sorted(new Document("iddepartament", -1)))
                            .title("Todos")
                            .name("findAll")
                            .build();
...
// </editor-fold>
    ...
    // <editor-fold defaultstate="collapsed" desc="String findByIdDepartament()">

    public String findByIdDepartament() {
        try {

    
            Bson filter = eq("iddepartament", departamentSelectedAutocomplete.getIddepartament());
            Document sort = new Document("departament", 1);
            ...
            paginator
                    = new Paginator.Builder()
                            ...
                            .filter(filter)
                            .sort(sort)
                            .sorted(new Sorted(new Document("departament", -1)))
                            .title("Todos")
                            .name("findAll")
                            .build();
...
// </editor-fold>
     // <editor-fold defaultstate="collapsed" desc="loadDepartament()">
    public void loadDepartament() {
        try {
            ...
            Document sort = new Document("departament", 1);
            departamentList = departamentServices.lookup(filter, sort, 0, 0);
        } catch (Exception e) {
            FacesUtil.errorMessage(FacesUtil.nameOfMethod() + "() : " + e.getLocalizedMessage());
        }
    }
...
if(dataTable!= null){
    ...
    // <editor-fold defaultstate="collapsed" desc="save(Departament departament)">
    public void save(Departament departament) {
        try {

            departamentSelected = departament;
            departamentSelected.setDepartament(departamentSelected.getDepartament().trim());
         departamentSelected.setCentral(centralSelected);
            if (departament.getDepartament() == null || departament.getDepartament().equals("")) {
                FacesUtil.warningDialog(rf.fromCore("warning.warning"), rf.fromMessage("warning.ingresedepartament"));
                return;
            }

           

            if (departamentServices.existsDepartament(departamentSelected,centralSelected)) {
            ...
            departamentSelected.setActionHistory(actionHistoryList);

  
            Optional<Departament> departamentOptional = departamentServices.save(departamentSelected);
            if (!departamentOptional.isPresent()) {
                FacesUtil.warningDialog(rf.fromCore("warning.save"), rf.fromCore("warning.save"));
            } else {
                departamentSelected = departamentOptional.get();
                JmoordbCoreContext.put("DepartamentFaces.departamentSelected", departamentSelected);
                ...
                closeOverlayPanel("PF('overlayPanelDepartament').hide()");
                refresh();
            }
...
// </editor-fold>
    // <editor-fold defaultstate="collapsed" desc="edit(Departament departament)">

    public void edit(Departament departament) {
        try {
            Departament departamentDBNow = departamentServices.findByIddepartament(departament.getIddepartament()).get();
            if (!departamentDBNow.equals(departamentDB)) {
            ...
            ActionHistory actionHistory = new ActionHistory.Builder()
                    ...
                    .build();
            List<ActionHistory> actionHistoryList = new ArrayList<>();
             actionHistoryList.addAll(departament.getActionHistory());
            actionHistoryList.add(actionHistory);

            departamentSelected.setActionHistory(actionHistoryList);

            Boolean isUpdate = departamentServices.update(departamentSelected);
            if (!isUpdate) {
                 ...
                 * Actualiza las tarjetas
                 */
                //  departamentList.add(departamentSelected);
                //  PrimeFaces.current().ajax().update("dataTable");
                closeOverlayPanel("PF('overlayPanelDepartament').hide()");
               // refresh();
            }
...
// </editor-fold>
    ...
    // <editor-fold defaultstate="collapsed" desc="void agregarRowDepartament()">
    public void agregarRowDepartament() {
        try {
            ...
            departamentSelected = new Departament();
            departamentSelected.setActive(Boolean.TRUE);
            isButtonSavePressed = Boolean.FALSE;
            tituloDialogo = rf.fromMessage("dialog.departamentagregar");
            PrimeFaces.current().ajax().update("outputPanelDepartament");
...
// </editor-fold>
    ...
    // <editor-fold defaultstate="collapsed" desc="String prepareDepartament(Departament departament)">
    public String prepareDepartament(Departament departament) {
        try {
            departamentSelected = new Departament();
            showDialogContent = Boolean.TRUE;
            departamentSelected = validarSiCambioDepartament(departament);
            isOverlayPanelOpen = Boolean.TRUE;
            isButtonSavePressed = Boolean.TRUE;
            buttonNewPressed = Boolean.FALSE;
            tituloDialogo = rf.fromMessage("dialog.departamenteditar");
        } catch (Exception e) {
            FacesUtil.errorMessage(FacesUtil.nameOfClassAndMethod() + " " + e.getLocalizedMessage());
        }
        return "";
...
// </editor-fold>
    ...
    // <editor-fold defaultstate="collapsed" desc="Departament validarSiCambioDepartament(Departament departament)">
    private Departament validarSiCambioDepartament(Departament departament) {
        Departament result = new Departament();
        try {
            message = "";
            fueCambiadoPorOtroUsuario = Boolean.FALSE;
            this.departamentDB = departamentServices.findByIddepartament(departament.getIddepartament()).get();
            if (departamentDB.equals(departament)) {
                result = departament;
            } else {
                fueCambiadoPorOtroUsuario = Boolean.TRUE;
                message = rf.fromMessage("warning.otrousuarioactualizodepartamentincronizeeltablero");
                result = departamentDB;
                showDialogContent = Boolean.FALSE;
        ...
        try {
            ...
            document.add(
                    ReportUtils.paragraph(
                            rf.fromMessage("print.titledepartament"), FontFactory.getFont("arial", 12, Font.BOLD), Element.ALIGN_CENTER));
...
//                            textoFiltro, FontFactory.getFont("arial", 10, Font.BOLD), Element.ALIGN_CENTER));
            ...
            table.addCell(
                    ReportUtils.PdfCell(
                            rf.fromMessage("print.iddepartament"), FontFactory.getFont("arial", 11, Font.BOLD), Element.ALIGN_CENTER));
            table.addCell(
                    ReportUtils.PdfCell(
...
//            
            ...
            for (int i = 1; i <= totalPage; i++) {
                ...
                List<Departament> list = departamentServices.lookup(paginator.getFilter(),
                ...
                if (list == null || list.isEmpty()) {
                    ...
                    list.forEach(c -> {
                        ...
                        table.addCell(
                                ReportUtils.PdfCell(
                                        c.getIddepartament().toString(), FontFactory.getFont("arial", 10, Font.NORMAL)));
                        table.addCell(
                                ReportUtils.PdfCell(
                                        c.getDepartament(), FontFactory.getFont("arial", 9, Font.NORMAL)));
String activo="";
if(c.getActive()){
    activo="si";
}else{
...
// <editor-fold defaultstate="collapsed" desc="List<Departament> completeDepartament(String query)">
    public List<Departament> completeDepartament(String query) {

        List<Departament> result = new ArrayList<>();
        try {
            query = query.trim();
          var list = departamentServices.likeByDepartament(query);
          if(list == null || list.isEmpty()){
              
          }else{
              list.stream().filter(d -> (d.getCentral().getIdcentral().equals(userLogged.getCentralView().getIdcentral()))).forEachOrdered(d -> {
                  result.add(d);
                });
          }
           
departamentConverterServices.add(result.subList(0,                     
...
// </editor-fold>
    ...
    // <editor-fold defaultstate="collapsed" desc="void autocompleteSelectedEvent(SelectEvent<Departament> event)">
    public void autocompleteSelectedEvent(SelectEvent<Departament> event) {
        try {
