...
import com.nerys.faces.services.RoleFacesServices;
...
@ViewScoped
@Data
public class RoleFaces implements Serializable, JmoordbCoreXHTMLUtil, IPaginator, RoleFacesServices , FacesServices{
// <editor-fold defaultstate="collapsed" desc="ConverterServices">
    ...
    private Role roleSelectedAutocomplete = new Role();
    private Role roleSelected = new Role();
    private Role roleDB = new Role();
    ...
    private Boolean haveRoleOpen = Boolean.FALSE;
    ...
    private LazyDataModel<Role> roleLazyDataModel;
// </editor-fold>
...
roleSelectedAutocomplete = new Role();
            ...
            this.roleLazyDataModel = new LazyDataModel<Role>() {
                @Override
                public List<Role> load(int offset, int pageSize, Map<String, SortMeta> sortBy, Map<String, FilterMeta> filterBy) {

                    switch (paginator.getName()) {
                        case "findAll":

                            totalRecords = roleServices.count(paginator.getFilter(),
...
//
                    ...
                    switch ((paginator.getName())) {
                        case "findAll":
                            ...
                            result = roleServices.lookup(paginator.getFilter(),
                    ...
                    roleLazyDataModel.setRowCount(totalRecords);
   ...
   @Override
                public String getRowKey(Role object) {
                    if (object == null || object.getIdrole()== null) {
                        return "";
                    }
                    return object.getIdrole().toString();
                }
        ...
        try {
            ...
            paginator
                    = new Paginator.Builder()
                            ...
                            .filter(filter)
                            .sort(sort)
                            .sorted(new Sorted(new Document("idrole", -1)))
                            .title("Todos")
                            .name("findAll")
                            .build();
...
// </editor-fold>
  // <editor-fold defaultstate="collapsed" desc="String findByIdRole()">

    public String findByIdRole() {
        try {

    
            Bson filter = eq("idrole", roleSelectedAutocomplete.getIdrole());
            Document sort = new Document("role", 1);
            ...
            paginator
                    = new Paginator.Builder()
                            ...
                            .filter(filter)
                            .sort(sort)
                            .sorted(new Sorted(new Document("role", -1)))
                            .title("Todos")
                            .name("findAll")
                            .build();
...
// </editor-fold>
  // <editor-fold defaultstate="collapsed" desc="String saveToMediaContext(String pathOfFile, String... nameOfFile)">
    ...
    // <editor-fold defaultstate="collapsed" desc="save(Role role)">
    public void save(Role role) {
        try {

            roleSelected = role;
            roleSelected.setRole(roleSelected.getRole().trim());
         
            if (role.getRole() == null || role.getRole().equals("")) {
                FacesUtil.warningDialog(rf.fromCore("warning.warning"), rf.fromMessage("warning.ingreserole"));
                return;
            }

           

            if (roleServices.existsRole(roleSelected)) {
            ...
            roleSelected.setActionHistory(actionHistoryList);

  
            Optional<Role> roleOptional = roleServices.save(roleSelected);
            if (!roleOptional.isPresent()) {
                FacesUtil.warningDialog(rf.fromCore("warning.save"), rf.fromCore("warning.save"));
            } else {
                roleSelected = roleOptional.get();
                JmoordbCoreContext.put("RoleFaces.roleSelected", roleSelected);
                ...
                closeOverlayPanel("PF('overlayPanelRole').hide()");
                refresh();
            }
...
// </editor-fold>
    // <editor-fold defaultstate="collapsed" desc="edit(Role role)">

    public void edit(Role role) {
        try {
            Role roleDBNow = roleServices.findByIdrole(role.getIdrole()).get();
            if (!roleDBNow.equals(roleDB)) {
            ...
            ActionHistory actionHistory = new ActionHistory.Builder()
                    ...
                    .build();
            List<ActionHistory> actionHistoryList = new ArrayList<>();
             actionHistoryList.addAll(role.getActionHistory());
            actionHistoryList.add(actionHistory);

            roleSelected.setActionHistory(actionHistoryList);

            Boolean isUpdate = roleServices.update(roleSelected);
            if (!isUpdate) {
...
// </editor-fold>
    ...
    // <editor-fold defaultstate="collapsed" desc="void agregarRowRole()">
    public void agregarRowRole() {
        try {
            ...
            roleSelected = new Role();
            roleSelected.setActive(Boolean.TRUE);
            isButtonSavePressed = Boolean.FALSE;
            tituloDialogo = rf.fromMessage("dialog.roleagregar");
            PrimeFaces.current().ajax().update("outputPanelRole");
...
// </editor-fold>
    ...
    // <editor-fold defaultstate="collapsed" desc="String prepareRole(Role role)">
    public String prepareRole(Role role) {
        try {
            roleSelected = new Role();
            showDialogContent = Boolean.TRUE;
            roleSelected = validarSiCambioRole(role);
            isOverlayPanelOpen = Boolean.TRUE;
            isButtonSavePressed = Boolean.TRUE;
            buttonNewPressed = Boolean.FALSE;
            tituloDialogo = rf.fromMessage("dialog.roleeditar");
        } catch (Exception e) {
            FacesUtil.errorMessage(FacesUtil.nameOfClassAndMethod() + " " + e.getLocalizedMessage());
        }
        return "";
...
// </editor-fold>
    ...
    // <editor-fold defaultstate="collapsed" desc="Role validarSiCambioRole(Role role)">
    private Role validarSiCambioRole(Role role) {
        Role result = new Role();
        try {
            message = "";
            fueCambiadoPorOtroUsuario = Boolean.FALSE;
            this.roleDB = roleServices.findByIdrole(role.getIdrole()).get();
            if (roleDB.equals(role)) {
                result = role;
            } else {
                fueCambiadoPorOtroUsuario = Boolean.TRUE;
                message = rf.fromMessage("warning.otrousuarioactualizoroleincronizeeltablero");
                result = roleDB;
                showDialogContent = Boolean.FALSE;
        ...
        try {
            ...
            document.add(
                    ReportUtils.paragraph(
                            rf.fromMessage("print.titlerole"), FontFactory.getFont("arial", 12, Font.BOLD), Element.ALIGN_CENTER));
...
//                            textoFiltro, FontFactory.getFont("arial", 10, Font.BOLD), Element.ALIGN_CENTER));
            ...
            table.addCell(
                    ReportUtils.PdfCell(
                            rf.fromMessage("print.idrole"), FontFactory.getFont("arial", 11, Font.BOLD), Element.ALIGN_CENTER));
            table.addCell(
                    ReportUtils.PdfCell(
                            rf.fromMessage("print.role"), FontFactory.getFont("arial", 11, Font.BOLD), Element.ALIGN_CENTER));
...
//            
            ...
            for (int i = 1; i <= totalPage; i++) {
                ...
                List<Role> list = roleServices.lookup(paginator.getFilter(),
                ...
                if (list == null || list.isEmpty()) {
                    ...
                    list.forEach(c -> {
                        ...
                        table.addCell(
                                ReportUtils.PdfCell(
                                        c.getIdrole().toString(), FontFactory.getFont("arial", 10, Font.NORMAL)));
                        table.addCell(
                                ReportUtils.PdfCell(
                                        c.getRole(), FontFactory.getFont("arial", 9, Font.NORMAL)));
String activo="";
if(c.getActive()){
    activo="si";
}else{
  ...
  // <editor-fold defaultstate="collapsed" desc="List<Role> completeRole(String query)">
    public List<Role> completeRole(String query) {

        List<Role> result = new ArrayList<>();
        try {
            query = query.trim();
            result = roleServices.likeByRole(query);
  roleConverterServices.add(result.subList(0,                     
...
// </editor-fold>
    ...
    // <editor-fold defaultstate="collapsed" desc="  autocompleteSelectedEvent(SelectEvent<Role> event)">
    public void autocompleteSelectedEvent(SelectEvent<Role> event) {
        try {
