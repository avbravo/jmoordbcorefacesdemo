...
import com.nerys.faces.services.CountryFacesServices;
...
@ViewScoped
@Data
public class CountryFaces implements Serializable, JmoordbCoreXHTMLUtil, IPaginator, CountryFacesServices, FacesServices {
// <editor-fold defaultstate="collapsed" desc="ConverterServices">
    ...
    private Country countrySelectedAutocomplete = new Country();
    private Country countrySelected = new Country();
    private Country countryDB = new Country();
    ...
    private Boolean haveCountryOpen = Boolean.FALSE;
    ...
    private LazyDataModel<Country> countryLazyDataModel;
// </editor-fold>
...
// </editor-fold>
    ...
    public void init() {
        try {
            ...
            countrySelectedAutocomplete = new Country();
            ...
            this.countryLazyDataModel = new LazyDataModel<Country>() {
                @Override
                public List<Country> load(int offset, int pageSize, Map<String, SortMeta> sortBy, Map<String, FilterMeta> filterBy) {

                    switch (paginator.getName()) {
                        case "findAll":

                            totalRecords = countryServices.count(paginator.getFilter(),
...
//
                    ...
                    switch ((paginator.getName())) {
                        case "findAll":
                            ...
                            result = countryServices.lookup(paginator.getFilter(),
                    ...
                    countryLazyDataModel.setRowCount(totalRecords);
                ...
                @Override
                public String getRowKey(Country object) {
                    if (object == null || object.getIdcountry() == null) {
                        return "";
                    }
                    return object.getIdcountry().toString();
                }
                ...
                public Country getRowData(String rowKey) {
                    for (Country t : countryList) {
                        if (t != null) {
                            if (t.getIdcountry().equals(rowKey)) {
                                return t;
                            }
                        }
                    }
...
// </editor-fold>
    ...
    public void preDestroy() {
        countryConverterServices.destroyed();
    }
        ...
        try {
            ...
            paginator
                    = new Paginator.Builder()
                            ...
                            .filter(filter)
                            .sort(sort)
                            .sorted(new Sorted(new Document("idcountry", -1)))
                            .title("Todos")
                            .name("findAll")
                            .build();
...
// </editor-fold>
    // <editor-fold defaultstate="collapsed" desc="String findByIdCountry()">

    public String findByIdCountry() {
        try {

            Bson filter = eq("idcountry", countrySelectedAutocomplete.getIdcountry());
            Document sort = new Document("country", 1);
            ...
            paginator
                    = new Paginator.Builder()
                            ...
                            .filter(filter)
                            .sort(sort)
                            .sorted(new Sorted(new Document("country", -1)))
                            .title("Todos")
                            .name("findAll")
                            .build();
...
// </editor-fold>
 // <editor-fold defaultstate="collapsed" desc="String saveToMediaContext(String pathOfFile, String... nameOfFile)">
    ...
    // <editor-fold defaultstate="collapsed" desc="save(Country country)">
    public void save(Country country) {
        try {

            countrySelected = country;
            countrySelected.setCountry(countrySelected.getCountry().trim());

            if (country.getCountry() == null || country.getCountry().equals("")) {
                FacesUtil.warningDialog(rf.fromCore("warning.warning"), rf.fromMessage("warning.ingresecountry"));
                return;
            }

            if (countryServices.existsCountry(countrySelected)) {
            ...
            countrySelected.setActionHistory(actionHistoryList);

            Optional<Country> countryOptional = countryServices.save(countrySelected);
            if (!countryOptional.isPresent()) {
                FacesUtil.warningDialog(rf.fromCore("warning.save"), rf.fromCore("warning.save"));
            } else {
                countrySelected = countryOptional.get();
                JmoordbCoreContext.put("CountryFaces.countrySelected", countrySelected);
                ...
                closeOverlayPanel("PF('overlayPanelCountry').hide()");
                refresh();
            }
...
// </editor-fold>
    // <editor-fold defaultstate="collapsed" desc="edit(Country country)">

    public void edit(Country country) {
        try {
            Country countryDBNow = countryServices.findByIdcountry(country.getIdcountry()).get();
            if (!countryDBNow.equals(countryDB)) {
            ...
            ActionHistory actionHistory = new ActionHistory.Builder()
                    ...
                    .build();
            List<ActionHistory> actionHistoryList = new ArrayList<>();
             actionHistoryList.addAll(country.getActionHistory());
            actionHistoryList.add(actionHistory);

            countrySelected.setActionHistory(actionHistoryList);

            Boolean isUpdate = countryServices.update(countrySelected);
            if (!isUpdate) {
                 ...
                 * Actualiza las tarjetas
                 */
                //  countryList.add(countrySelected);
                //  PrimeFaces.current().ajax().update("dataTable");
                closeOverlayPanel("PF('overlayPanelCountry').hide()");
                // refresh();
            }
...
// </editor-fold>
    ...
    // <editor-fold defaultstate="collapsed" desc="void agregarRowCountry()">
    public void agregarRowCountry() {
        try {
            ...
            countrySelected = new Country();
            countrySelected.setIdcountry("");
            countrySelected.setCountry("");
            isButtonSavePressed = Boolean.FALSE;
            tituloDialogo = rf.fromMessage("dialog.countryagregar");
            PrimeFaces.current().ajax().update("outputPanelCountry");
...
// </editor-fold>
    ...
    // <editor-fold defaultstate="collapsed" desc="String prepareCountry(Country country)">
    public String prepareCountry(Country country) {
        try {
            countrySelected = new Country();
            showDialogContent = Boolean.TRUE;
            countrySelected = validarSiCambioCountry(country);
            isOverlayPanelOpen = Boolean.TRUE;
            isButtonSavePressed = Boolean.TRUE;
            buttonNewPressed = Boolean.FALSE;
            tituloDialogo = rf.fromMessage("dialog.countryeditar");
        } catch (Exception e) {
            FacesUtil.errorMessage(FacesUtil.nameOfClassAndMethod() + " " + e.getLocalizedMessage());
        }
        return "";
...
// </editor-fold>
    ...
    // <editor-fold defaultstate="collapsed" desc="Country validarSiCambioCountry(Country country)">
    private Country validarSiCambioCountry(Country country) {
        Country result = new Country();
        try {
            message = "";
            fueCambiadoPorOtroUsuario = Boolean.FALSE;
            this.countryDB = countryServices.findByIdcountry(country.getIdcountry()).get();
            if (countryDB.equals(country)) {
                result = country;
            } else {
                fueCambiadoPorOtroUsuario = Boolean.TRUE;
                message = rf.fromMessage("warning.otrousuarioactualizocountryincronizeeltablero");
                result = countryDB;
                showDialogContent = Boolean.FALSE;
        ...
        try {
            ...
            document.add(
                    ReportUtils.paragraph(
                            rf.fromMessage("print.titlecountry"), FontFactory.getFont("arial", 12, Font.BOLD), Element.ALIGN_CENTER));
...
//                            textoFiltro, FontFactory.getFont("arial", 10, Font.BOLD), Element.ALIGN_CENTER));
            ...
            table.addCell(
                    ReportUtils.PdfCell(
                            rf.fromMessage("print.idcountry"), FontFactory.getFont("arial", 11, Font.BOLD), Element.ALIGN_CENTER));
            table.addCell(
                    ReportUtils.PdfCell(
                            rf.fromMessage("print.country"), FontFactory.getFont("arial", 11, Font.BOLD), Element.ALIGN_CENTER));
...
//            
            ...
            for (int i = 1; i <= totalPage; i++) {
                ...
                List<Country> list = countryServices.lookup(paginator.getFilter(),
                ...
                if (list == null || list.isEmpty()) {
                    ...
                    list.forEach(c -> {
                        ...
                        table.addCell(
                                ReportUtils.PdfCell(
                                        c.getIdcountry(), FontFactory.getFont("arial", 10, Font.NORMAL)));
                        table.addCell(
                                ReportUtils.PdfCell(
                                        c.getCountry(), FontFactory.getFont("arial", 9, Font.NORMAL)));
    ...
    // </editor-fold>
    // <editor-fold defaultstate="collapsed" desc="List<Country> completeCountry(String query)">
    public List<Country> completeCountry(String query) {

        List<Country> result = new ArrayList<>();
        try {
            query = query.trim();
            result = countryServices.likeByCountry(query);
            countryConverterServices.add(result.subList(0,
...
// </editor-fold>
    ...
    // <editor-fold defaultstate="collapsed" desc="  autocompleteSelectedEvent(SelectEvent<Country> event)">
    public void autocompleteSelectedEvent(SelectEvent<Country> event) {
        try {
