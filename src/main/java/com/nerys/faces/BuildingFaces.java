...
import com.nerys.faces.services.BuildingFacesServices;
...
@ViewScoped
@Data
public class BuildingFaces implements Serializable, JmoordbCoreXHTMLUtil, IPaginator, BuildingFacesServices, FacesServices {
// <editor-fold defaultstate="collapsed" desc="ConverterServices">
 ...
 @Inject
    ...
    private Building buildingSelected = new Building();
    private Building buildingSelectedAutocomplete = new Building();
    private Building buildingDB = new Building();
    ...
    private Boolean haveBuildingOpen = Boolean.FALSE;
    ...
    private LazyDataModel<Building> buildingLazyDataModel;
// </editor-fold>
...
// </editor-fold>
    ...
    public void init() {
        try {
            haveBuildingOpen = Boolean.FALSE;
            message = "";
            ...
            buildingSelectedAutocomplete = new Building();
            ...
            this.buildingLazyDataModel = new LazyDataModel<Building>() {
                @Override
                public List<Building> load(int offset, int pageSize, Map<String, SortMeta> sortBy, Map<String, FilterMeta> filterBy) {

                    switch (paginator.getName()) {
                        case "findAll":

                            totalRecords = buildingServices.count(paginator.getFilter(),
...
//
                    ...
                    switch ((paginator.getName())) {
                        case "findAll":
                            ...
                            result = buildingServices.lookup(paginator.getFilter(),
                    ...
                    buildingLazyDataModel.setRowCount(totalRecords);
                ...
                @Override
                public String getRowKey(Building object) {
                    if (object == null || object.getIdbuilding() == null) {
                        return "";
                    }
                    return object.getIdbuilding().toString();
                }

                @Override
                public Building getRowData(String rowKey) {
                    for (Building t : buildingList) {
                        if (t != null) {
                            if (t.getIdbuilding().equals(rowKey)) {
                                return t;
                            }
                        }
                    }
        ...
        try {
            ...
            paginator
                    = new Paginator.Builder()
                            ...
                            .filter(filter)
                            .sort(sort)
                            .sorted(new Sorted(new Document("idbuilding", -1)))
                            .title("Todos")
                            .name("findAll")
                            .build();
...
// </editor-fold>
    ...
    public String findByIdBuilding() {
        try {

            Bson filter0 = or(eq("active", Boolean.FALSE),
                    eq("active", Boolean.TRUE)
            );
            Bson filter = and(filter0, eq("idbuilding", buildingSelectedAutocomplete.getIdbuilding()));
            Document sort = new Document("building", 1);
            ...
            paginator
                    = new Paginator.Builder()
                            ...
                            .filter(filter)
                            .sort(sort)
                            .sorted(new Sorted(new Document("building", -1)))
                            .title("Todos")
                            .name("findAll")
                            .build();
...
// </editor-fold>
 ...
 // <editor-fold defaultstate="collapsed" desc="String saveToMediaContext(String pathOfFile, String... nameOfFile)">
    ...
    // <editor-fold defaultstate="collapsed" desc="save(Building building)">
    public void save(Building building) {
        try {

            buildingSelected = building;
            buildingSelected.setActive(Boolean.TRUE);
            buildingSelected.setBuilding(buildingSelected.getBuilding().trim());
            if (buildingSelected.getCentral() == null || buildingSelected.getCentral().getIdcentral() == null) {
                FacesUtil.warningDialog(rf.fromCore("warning.warning"), rf.fromMessage("warning.seleccionecentral"));
                return;
            }

            if (building.getBuilding() == null || building.getBuilding().equals("")) {
                FacesUtil.warningDialog(rf.fromCore("warning.warning"), rf.fromMessage("warning.ingresebuilding"));
                return;
            }

            if (buildingServices.existsBuilding(buildingSelected)) {
            ...
            buildingSelected.setActionHistory(actionHistoryList);

            Optional<Building> buildingOptional = buildingServices.save(buildingSelected);
            if (!buildingOptional.isPresent()) {
                FacesUtil.warningDialog(rf.fromCore("warning.save"), rf.fromCore("warning.save"));
            } else {
                buildingSelected = buildingOptional.get();
                JmoordbCoreContext.put("BuildingFaces.buildingSelected", buildingSelected);
                ...
                closeOverlayPanel("PF('overlayPanelBuilding').hide()");
                refresh();
            }
...
// </editor-fold>
    // <editor-fold defaultstate="collapsed" desc="edit(Building building)">

    public void edit(Building building) {
        try {
            Building buildingDBNow = buildingServices.findByIdbuilding(building.getIdbuilding()).get();
            if (!buildingDBNow.equals(buildingDB)) {
             ...
             *
             */
            if (buildingServices.existsBuildingWithDiferentId(buildingSelected)) {
            ...
            ActionHistory actionHistory = new ActionHistory.Builder()
                    ...
                    .build();
            List<ActionHistory> actionHistoryList = new ArrayList<>();
             actionHistoryList.addAll(building.getActionHistory());
            actionHistoryList.add(actionHistory);

            buildingSelected.setActionHistory(actionHistoryList);

            Boolean isUpdate = buildingServices.update(buildingSelected);
            if (!isUpdate) {
                 ...
                 * Actualiza las tarjetas
                 */
                //  buildingList.add(buildingSelected);
                //  PrimeFaces.current().ajax().update("dataTable");
                closeOverlayPanel("PF('overlayPanelBuilding').hide()");
                //refresh();
            }
...
// </editor-fold>
    ...
    // <editor-fold defaultstate="collapsed" desc="void agregarRowBuilding()">
    public void agregarRowBuilding() {
        try {
            ...
            PrimeFaces.current().ajax().update("outputPanelBuilding");
...
// </editor-fold>
    ...
    // <editor-fold defaultstate="collapsed" desc="String prepareBuilding(Building building)">
    public String prepareBuilding(Building building) {
        try {
            buildingSelected = new Building();
            buildingSelected.setActive(Boolean.TRUE);
            showDialogContent = Boolean.TRUE;
            buildingSelected = validarSiCambioBuilding(building);
...
// </editor-fold>
    ...
    // <editor-fold defaultstate="collapsed" desc="Building validarSiCambioBuilding(Building building)">
    private Building validarSiCambioBuilding(Building building) {
        Building result = new Building();
        try {
            message = "";
            fueCambiadoPorOtroUsuario = Boolean.FALSE;
            this.buildingDB = buildingServices.findByIdbuilding(building.getIdbuilding()).get();
            if (buildingDB.equals(building)) {
                result = building;
            } else {
                fueCambiadoPorOtroUsuario = Boolean.TRUE;
                message = rf.fromMessage("warning.otrousuarioactualizobuildingincronizeeltablero");
                result = buildingDB;
                showDialogContent = Boolean.FALSE;
        ...
        try {
            ...
            document.add(
                    ReportUtils.paragraph(
                            rf.fromMessage("print.titlebuilding"), FontFactory.getFont("arial", 12, Font.BOLD), Element.ALIGN_CENTER));
...
//                            textoFiltro, FontFactory.getFont("arial", 10, Font.BOLD), Element.ALIGN_CENTER));
            ...
            table.addCell(
                    ReportUtils.PdfCell(
                            rf.fromMessage("print.idbuilding"), FontFactory.getFont("arial", 11, Font.BOLD), Element.ALIGN_CENTER));
...
//            
            ...
            for (int i = 1; i <= totalPage; i++) {
                ...
                List<Building> list = buildingServices.lookup(paginator.getFilter(),
                ...
                if (list == null || list.isEmpty()) {
                    ...
                    list.forEach(c -> {
                        ...
                        table.addCell(
                                ReportUtils.PdfCell(
                                        c.getIdbuilding().toString(), FontFactory.getFont("arial", 10, Font.NORMAL)));

                        table.addCell(
                                ReportUtils.PdfCell(
                                        c.getBuilding(), FontFactory.getFont("arial", 9, Font.NORMAL)));
    ...
    // </editor-fold>
    // <editor-fold defaultstate="collapsed" desc="List<Building> completeBuilding(String query)">
    public List<Building> completeBuilding(String query) {

        List<Building> result = new ArrayList<>();
        try {
            query = query.trim();
            result = buildingServices.likeByBuilding(query);
                                   
            buildingConverterServices.add(result.subList(0,                     
...
// </editor-fold>
    ...
    public void autocompleteSelectedEvent(SelectEvent<Building> event) {
        try {
